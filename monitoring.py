...
def register_site(site: Site) -> None:
    ...
    ----------
    site : Site
        A fully configured :class:`Site` dataclass instance.
    """
...
def get_site(name: str) -> Site:
    ...
    ----------
    name : str
        The ``site.name`` used when ``register_site`` was called.

    Returns
    -------
    Site
        The registered :class:`Site` instance.

    Raises
    ------
    KeyError
        When no site with *name* has been registered.
    """
    return _SITES[name]
...
def set_active_site(site: Site) -> None:
    ...
    All engine functions that depend on site configuration (channel lists,
    path roots, callbacks) read ``_active_site`` instead of reaching into
    ``config.py`` directly.

    Parameters
    ----------
    site : Site
        A :class:`Site` instance that has already been passed to
        :func:`register_site`.
    """
    global _active_site
    _active_site = site
...
def get_active_site() -> Optional[Site]:
    ...
    -------
    Site or None
        The active :class:`Site` instance, or ``None`` if
        :func:`set_active_site` has not been called yet.
    """
    return _active_site
...
def get_file_list(origin, reduced=False, file_info=None):
    ...
    ----------
    origin : str
        Origin tag (e.g. ``'accel'``, ``'wind'``).
    reduced : bool, optional
        When ``True``, the callback should return only files not already
        present in *file_info* (incremental update mode).
    file_info : xarray.Dataset or None, optional
        The current file-info database; used by the callback when
        *reduced* is ``True`` to filter already-indexed files.
    ...
    ------
    RuntimeError
        When no site has been registered (``_active_site`` is ``None``)
        or the site has no ``file_list_fn``.
    """
    site = _active_site
...
def get_file_info(origin: str, create_new: bool=False, **kwargs):
    ...
    ----------
    origin : str
        Origin tag identifying which sensor group to query.
    create_new : bool, optional
        When ``True``, call :func:`create_file_info` instead of reading
        from disk.  Defaults to ``False``.
    **kwargs
        Forwarded to :func:`create_file_info` when *create_new* is
        ``True``.

    Returns
    -------
    xarray.Dataset or None
        The file-info dataset with an additional ``gap_length`` variable
        (samples between consecutive files), or ``None`` if the on-disk
        database is newly created and empty.
    """
    site = _active_site
...
def close_to_utc_transition(file_time, close_hours=3):
    ...
    Thin wrapper around :meth:`TimeConvention.is_near_dst_transition` that
    also emits an ``INFO`` log message when the guard fires.

    Parameters
    ----------
    file_time : datetime.datetime
        The timestamp to check.
    close_hours : int, optional
        Half-width of the exclusion window in hours.  Defaults to 3.

    Returns
    -------
    bool
        ``True`` when *file_time* is within ±*close_hours* of any DST
        transition in the project timezone.
    """
...
def round_dt(dt: np.datetime64, duration: np.timedelta64,
    ...
    duration : numpy.timedelta64
        Rounding grid size (e.g. ``np.timedelta64(120, 'm')`` for 2-hour
        intervals).
    ceil : bool, optional
        Round toward the next grid point (default ``True``).
    floor : bool, optional
        Round toward the previous grid point.  Setting this to ``True``
        forces *ceil* to ``False``.
...
def describe_stats(measurement, headers=None, quantity=None):
    ...
    For each column of *measurement* the following statistics are computed:
    mean, min, max, variance, skewness, kurtosis, 5th/50th/95th percentiles,
    and RMS.  An ``error`` flag is set when a channel contains only NaN values,
    has min == max (flatline), or violates the plausibility range defined in
    the active site's ``ranges`` dict.

    Parameters
    ----------
    measurement : numpy.ndarray, shape (n_samples, n_channels)
        Raw or preprocessed signal matrix.  May contain ``NaN`` values.
    headers : list of str, optional
        Channel names in the same order as columns in *measurement*.  Used
        to look up plausibility ranges and site-specific circular-mean
        functions.  Defaults to empty strings.
    quantity : str, optional
        Quantity tag (e.g. ``'accel'``) used to look up
        ``error_rules`` (kurtosis thresholds) in the active site.

    Returns
    -------
    dict
        Keys: ``'mean'``, ``'min'``, ``'max'``, ``'var'``, ``'skewness'``,
        ``'kurtosis'``, ``'q05'``, ``'q50'``, ``'q95'``, ``'rms'``,
        ``'error'``.  Each value is a 1-D ``numpy.ndarray`` of length
        ``n_channels``.
    """
    if headers is None:
...
def check_and_mark_errors(ds, new = True, check_kurtosis = False):
    """Check signal plausibility and update the ``error`` flag in a dataset.

    Parameters
    ----------
    ds : xarray.Dataset
        A statistics or modal-results dataset that has ``min``, ``max``,
        ``kurtosis``, and ``error`` data variables with a ``channels``
        coordinate.
    new : bool, optional
        When ``True`` (default), discard any pre-existing ``error`` flags
        and compute them from scratch.  When ``False``, increment the
        existing flags (i.e. OR new detections with old ones).
    check_kurtosis : bool, optional
        When ``True``, flag channels whose kurtosis falls outside
        ``[-2, 5]``.  Defaults to ``False``.

    Returns
    -------
    xarray.Dataset
        *ds* with its ``error`` variable updated in-place.
    """
...
def get_stats(quantity: str, duration: pd.Timedelta,
    ...
    duration : pandas.Timedelta
        Analysis window length (e.g. ``pd.Timedelta('2h')`` for 120-minute
        statistics).
    file_info : xarray.Dataset or None, optional
        File-info dataset required when *create_new* is ``True``.
    create_new : bool, optional
        When ``True``, run :func:`create_stats` instead of reading from
        the existing NetCDF file.  Defaults to ``False``.
    **kwargs
        Forwarded to :func:`create_stats` when *create_new* is ``True``.

    Returns
    -------
    xarray.Dataset
        Statistics dataset with variables ``mean``, ``min``, ``max``,
        ``var``, ``skewness``, ``kurtosis``, ``q05``, ``q50``, ``q95``,
        ``rms``, and ``error`` indexed by ``(time, channels)``.

    Raises
    ------
    RuntimeError
        When *create_new* is ``True`` but *file_info* is ``None``.
    """
    site = _active_site
...
def get_slice_corrected(start_time: pd.Timestamp, duration: pd.Timedelta,
    ...
    First checks whether a pre-processed ``.npz`` slice already exists on
    disk (under ``slice_root_path/<duration>-minutes/slices_<quantity>/``).
    If it does, the cached slice is returned directly.  Otherwise, the raw
    slice is obtained via :func:`get_slice`, the site's ``transforms``
    callback is applied, and the result is saved for future calls.

    Parameters
    ----------
    start_time : pandas.Timestamp
        Berlin-aware start of the requested window.
    duration : pandas.Timedelta
        Length of the requested window.
    quantity : str
        Quantity tag (e.g. ``'accel'``, ``'strain_rosettes'``).
    file_info : xarray.Dataset or None, optional
        File-info dataset required when the cache is empty and a raw slice
        must be assembled.  May be ``None`` if a cached slice exists.
    **kwargs
        Forwarded to the site's ``transforms`` callback.

    Returns
    -------
    tuple or None
        ``(start_time, headers, units, end_time, sample_rate, measurement)``
        or ``None`` when no data are available for the requested window.
    """
    st = start_time
...
def get_modal_results(quantity: str, duration: pd.Timedelta,
    ...
    quantity : str
        Quantity tag (e.g. ``'accel'``, ``'strain_rosettes'``).
    duration : pandas.Timedelta
        Analysis window length used to locate the database file.
    stats : xarray.Dataset or None, optional
        Statistics dataset required when *create_new* is ``True``.
    create_new : bool, optional
        When ``True``, run :func:`create_modal_results` instead of reading
        from disk.  Defaults to ``False``.
    **kwargs
        Forwarded to :func:`create_modal_results` when *create_new* is
        ``True``.

    Returns
    -------
    xarray.Dataset
        Modal-results dataset with variables ``frequencies``, ``damping``,
        ``modeshapes``, ``MPC``, ``MPD``, ``num_modes``, ``model_orders``,
        and associated per-channel PSD statistics, indexed by
        ``(time, modes, channels)``.

    Raises
    ------
    RuntimeError
        When *create_new* is ``True`` but *stats* is ``None``.
    """
    site = _active_site
    minutes = int(duration.total_seconds()/60)
...
def create_modal_results(quantity: str, duration: pd.Timedelta,
    ...
    For each time step that is not already in the master database the
    function calls, in order:

    1. :func:`get_slice_preprocessed` — bandpass-filter and decimate the
       raw slice.
    2. :func:`describe_stats` — compute per-channel signal statistics.
    3. :func:`modal_analysis_single` — run VarSSI-Ref, cluster the
       stabilization diagram, and auto-select physical poles.

    Results are accumulated in a per-process NetCDF file and then merged
    into the master database under :class:`MultiLock`.
    ...
    stats : xarray.Dataset
        Statistics dataset (from :func:`get_stats`) providing the list of
        time steps to process and error flags to filter on.
    chunksize : int, optional
        Number of windows to accumulate before flushing the per-process
        NetCDF file to disk.  Defaults to 2.
    skip_existing : bool, optional
        When ``True`` (default), skip time steps already present in the
        master database.
    check_errors : bool, optional
        When ``True`` (default), call :func:`check_and_mark_errors` on
        *stats* before iterating.
    filter_errors : bool, optional
        When ``True`` (default), skip time steps that have any error flag.
    **kwargs
        Supported keys: ``dtstart``, ``until``, ``missing``,
        ``num_workers``, ``this_worker``.
...
def modal_analysis_single(start_time, data_slice, quantity, duration):
    ...
    1. ``prep_data.npz`` — :class:`~pyOMA.core.PreProcessingTools.PreProcessSignals`
       state including the singular-value PSD.
    2. ``modal_data.npz`` — :class:`~pyOMA.core.VarSSIRef.VarSSIRef` state
       matrix.
    3. ``stabil_data.npz`` — :class:`~pyOMA.core.StabilDiagram.StabilCluster`
       state with automatic clearing, classification, and pole selection.
    ...
    ----------
    start_time : pandas.Timestamp
        Berlin-aware start of the analysis window (used to name result files
        and locate the configuration directory).
    data_slice : tuple
        ``(start_time, headers, units, end_time, sample_rate, measurement)``
        as returned by :func:`get_slice_preprocessed`.
    quantity : str
        Quantity tag (e.g. ``'accel'``); used to locate the
        ``modal_conf_dir/<quantity>/`` configuration directory.
    duration : pandas.Timedelta
        Window length; determines the result subfolder name.

    Returns
    -------
    list
        ``[model_orders, frequencies, std_frequencies, damping, std_damping,
        MPC, MP, MPD, MC, modeshapes, s_vals_psd]`` as returned by
        :meth:`~pyOMA.core.StabilDiagram.StabilCluster.return_results` plus
        the singular-value PSD matrix.

    Raises
    ------
    RuntimeError
        When no ``setup_prep`` callback is registered for *quantity*.
    """
    site = _active_site
...
def split_modepairs(modal):
    ...
    2. Assigns the lower-frequency pole to slot ``2*i`` and the
       higher-frequency pole to slot ``2*i+1`` (where *i* is the band index).
    3. Merges all per-band results into a single dataset with a new
       ``modes`` coordinate of size ``2 * len(modal_bands)``.

    All other dataset variables (damping, modeshapes, MPC, etc.) are
    reordered to match the new mode assignment.

    Parameters
    ----------
    modal : xarray.Dataset
        Raw modal-results dataset as returned by :func:`get_modal_results`,
        with a sparse ``modes`` dimension.

    Returns
    -------
    xarray.Dataset
        Dataset with a dense ``modes`` coordinate of length
        ``2 * len(site.modal_bands)``, aligned across all time steps.
    """
    site = _active_site
