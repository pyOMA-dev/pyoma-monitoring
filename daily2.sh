...
if test -f "${TMPDIR}/dtstart.tmp"; then
	...
	cat ${TMPDIR}/geyer_out.txt | mail -s "Structural Monitoring Tower: Daily Analysis of Temperature Records" -a ${TMPDIR}/stats_temp_10.png ${RECIPIENTS}
else
	cat ${TMPDIR}/geyer_out.txt | mail -s "Structural Monitoring Tower: Daily Analysis of Temperature Records" ${RECIPIENTS}
fi
rm -rf ${TMPDIR}
...
if test -f "${TMPDIR}/dtstart.tmp"; then
	...
	cat ${TMPDIR}/geyer_out.txt | mail -s "Structural Monitoring Tower: Daily Analysis of Wind Records" -a ${TMPDIR}/stats_wind_10.png ${RECIPIENTS}
else
	cat ${TMPDIR}/geyer_out.txt | mail -s "Structural Monitoring Tower: Daily Analysis of Wind Records" ${RECIPIENTS}
fi
rm -rf ${TMPDIR}
...
if test -f "${TMPDIR}/dtstart.tmp"; then
	...
	cat ${TMPDIR}/geyer_out.txt | mail -s "Structural Monitoring Tower: Daily Analysis of Acceleration Records" -a ${TMPDIR}/stats_accel_10.png -a ${TMPDIR}/modal_accel_10.png -a ${TMPDIR}/spec_accel_10.png ${RECIPIENTS}
else
	cat ${TMPDIR}/geyer_out.txt | mail -s "Structural Monitoring Tower: Daily Analysis of Acceleration Records" ${RECIPIENTS}
fi
rm -rf ${TMPDIR}
