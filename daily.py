...
def main(argv):
    ...
    if file_info:
        fi_ds = get_file_info(path, subpath, origin, create_new=False)
        
        filtered_list = get_file_list(os.path.join(path,subpath), origin, True, fi_ds)
        ...
        fi_ds = get_file_info(path, subpath, origin, create_new=True, skip_existing=True, reduced=True, filtered_list=True)
    ...
    if stats:
        ...
        stats_ds = get_stats(path, quantity, fi_ds, duration = pd.Timedelta(minutes=duration), dtstart=dtstart, 
                             create_new=True, skip_existing=True, chunksize=500)#, until=until)
    else:
        stats_ds = get_stats(path, quantity)
    
    if modal: 
        modal_ds = get_modal_results(path, quantity, stats_ds, 
                                  skip_existing=True, create_new=True, 
                                  filter_errors=False, 
                                  chunksize=50, missing=True)
        
    if plots:
        fig1, fig2 = plot_daily(path, quantity, duration, dtstart)
